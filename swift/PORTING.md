...
| 7 | `enrichOracle` (translation + IPA) | IPA via **espeak** subprocess (`/opt/local/bin/espeak`, present) when available; translation via a bundled dictionary seed / Apple `Translation` (deferred), else no-op. Behind `EnrichOracle`. | Keeps the autofill port working offline; real translation is a later swap that preserves the port signature (per `co-development.md`). |
