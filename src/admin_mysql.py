...
def create_session(user_id: int, ip_address: str = "unknown", username: str = None,
                   user_agent: str = "unknown", app_name: str = "app") -> Optional[str]:
    """
    Create a new session for authenticated user in unified sessions table.
    Uses bootstrap connection before user's tracked connection is established.
    Includes all tracking metadata (device, browser, app) in single table.
    
    Args:
        user_id: User ID
        ip_address: Client IP address for security tracking
        username: Username (optional, will be looked up if not provided)
        user_agent: Browser user agent string
        app_name: Which app created the session ('app', 'miolingo-admin', 'connection_monitor')
    ...
    try:
        ...
        with pool.get_bootstrap_connection() as conn:
            ...
            # Insert into unified sessions table with all metadata
            query = """
            ...
            """
            cursor.execute(query, (session_id, user_id, username, expires_at, ip_address, 
                                   user_agent, device_type, browser, app_name))
            conn.commit()
            cursor.close()
...
def log_session_to_monitor(session_id: str, username: str, user_ip: str, user_agent: str, app_name: str) -> bool:
    """
    DEPRECATED: Session metadata now stored directly in unified sessions table.
    This function is kept for backward compatibility but does nothing.
    Use create_session() with full parameters instead.
    ...
    """
    # Session metadata is now in unified sessions table
    # No separate session_monitor table needed
    return True
