...
**Component inventory correction.** The read-only "displays" (Stats Display, History Browser) are most likely **view ports on** the stateful agents (PracticeSession / Vocab), not standalone L1 agents. Promote one to a standalone agent only if it genuinely owns state. Decide per component.
