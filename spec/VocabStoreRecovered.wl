...
(* =====================================================================
   ...
   FORM: compact guarded-choice (one summand per port, `when`-guarded).
   Single source of truth; guard-partitioned normal form is a DERIVED view.
   `afforded` removed (readiness = analysis-only, readyPorts[state]);
   `view` kept. Sugar from discipline.wl. LOAD ORDER: RCA_core.wl,
   discipline.wl, then this.

   STATE: VS[auth, entries, sort, filter, editing]
     auth : anon | signedIn     entries : list     sort : alpha|recent|oldest
     filter : none | filterBy[q]   editing : none | editing[id]
   ...
   READY SETS (analysis; computed by readyPorts, not declared):
     ...
     VS[signedIn, ne, _,_,editing[id]] : update, cancel_edit replace the
...
(* --- top: view always; domain ports only when signed in (auth gate) --- *)
